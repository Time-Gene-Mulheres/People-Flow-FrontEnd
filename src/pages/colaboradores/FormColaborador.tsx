    ...
    async function gerarNovoColaborador(e: ChangeEvent<HTMLFormElement>) {
        ...
        if (id !== undefined) {
            try {
                ...
                ToastAlerta("O cadastro do colaborador foi atualizado com sucesso!", "sucesso")
            } catch (error: any) {
                if (error.toString().includes('403')) {
                    handleLogout();
                } else {
    ...
    return (
        <div className="container flex flex-col items-center justify-center mx-auto">
            <h1 className="text-4xl text-center my-8">
                {id === undefined ? 'Cadastrar colaborador' : 'Editar cadastro colaborador'}
            </h1>

            <form className="w-1/2 flex flex-col gap-4" onSubmit={gerarNovoColaborador}>
                <div className="flex flex-col gap-2">
                    <label htmlFor="descricao">Dados do colaborador</label>
                    <input
                        type="text"
                        placeholder="Insira aqui os dados do colaborador"
                        name='descricao'
